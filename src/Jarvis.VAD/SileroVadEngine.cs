...
public sealed class SileroVadEngine : IDisposable
{
    ...
    public VadFrameResult Process(ReadOnlySpan<short> pcm16)
    {
        ...
        var inputData = new float[FrameSamples];
        for (var i = 0; i < pcm16.Length; i++) inputData[i] = pcm16[i] / 32768f;

        var input = new DenseTensor<float>(inputData, new[] { 1, FrameSamples });
