...
Lightroom Classic 15.5.1: Jarvis Lua plug-in + loopback-only TCP bridge are live. Get/set/relative adjustment commands are verified for Exposure, Contrast, Highlights, Shadows, Whites, Blacks, Texture, Clarity, Dehaze, Vibrance, Saturation, Temperature and Tint. Auto Tone and Auto White Balance are implemented through the SDK but are not fired automatically in smoke tests because they alter multiple develop settings. Develop copy/paste and native undo/redo are LIVE PASS. Navigation next/previous, rating 0-5, rating up/down, and Pick/Reject/clear flags are also verified live through `LrSelection`, with reversible state restoration.
